...
def calc_dc_measures_normalization(path_to_datasets, norm_type):
    data_list, label_list, fname = load_data(path_to_datasets)
    total_dcm = []
    for index, file in enumerate(fname):
        print('\tFile:\t' + file + '...', flush=True)
...
if __name__ == '__main__':
    ...
    total_dc_df_1.to_csv(path_to_saved_csvs+"normalized_dc_measures.csv")

    total_dc_df_2 = calc_dc_measures(path_to_datasets, is_normalized=True, is_resampling=True)
    total_dc_df_2.to_csv(path_to_saved_csvs + "norm_res_dc_measures.csv")

    total_dc_df_3 = calc_dc_measures(path_to_datasets, is_normalized=True, is_resampling=True, is_feature_selection=True)
    total_dc_df_3.to_csv(path_to_saved_csvs + "norm_res_fs_dc_measures.csv")
